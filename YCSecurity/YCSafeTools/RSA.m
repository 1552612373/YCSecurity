...
+ (NSData *)encryptData:(NSData *)data withKeyRef:(SecKeyRef) keyRef isSign:(BOOL)isSign {
	...
	for(int idx=0; idx<srclen; idx+=src_block_size){
        ...
        // 后台的加密模式RSA/ECB/OAEPWithSHA-1AndMGF1Padding；对应的kSecPaddingPKCS1要修改为kSecPaddingOAEP
        if (isSign) {
...
+ (NSData *)decryptData:(NSData *)data withKeyRef:(SecKeyRef) keyRef{
	...
	for(int idx=0; idx<srclen; idx+=src_block_size){
		...
		size_t outlen = block_size;
		OSStatus status = noErr;
        // 后台的加密模式RSA/ECB/OAEPWithSHA-1AndMGF1Padding；对应的kSecPaddingNone要修改为kSecPaddingOAEP
